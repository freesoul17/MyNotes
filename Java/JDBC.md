...
- **几种常用数据库的 JDBC URL**
  ...
  - MySQL的连接URL编写方式：
    ...
    - jdbc:mysql://localhost:3306/DatabaseName**?useUnicode=true&characterEncoding=utf8**（如果JDBC程序与服务器端的字符集不一致，会导致乱码，那么可以通过参数指定服务器端的字符集）
    - jdbc:mysql://localhost:3306/DatabaseName?user=root&password=123456
...
//        5.预编译sql语句，返回PreparedStatement实例
        String sql = "insert into stu(name,id)values(?,?)";
...
//        6.执行，并返回结果集
        rs = ps.executeQuery();
//        7.处理结果集
        if (rs.next())//next()判断结果集的下一条是否有数据，如果有，指针下移
        {
            int id = rs.getInt(1);
...
由于每次都需要连接数据库、关闭资源且是固定的，增删改操作只有SQL语句和占位符不一样，查的操作也是相对固定的，所以创建一个工具类
...
public class GetBlob {
    public static void main(String[] args) throws Exception {
        ...
        String sql = "select photo from nice where id = 1";
        PreparedStatement ps = conn.prepareStatement(sql);
        ResultSet rs = ps.executeQuery();
        while (rs.next()) {
            Blob photo = rs.getBlob(1);
            InputStream is2 = photo.getBinaryStream();
